    ...
    <TooltipPrimitive.Portal>
      <TooltipPrimitive.Content
        ...
        className={cn(
          ...
          'z-50 w-fit text-balance rounded-md px-3 py-1.5',
          'bg-surface-inverse text-content-inverse text-caption',
...
export interface HeroCardTooltipProps {
  title: string;
  href: string;
  imgSrc: string;
  imgAlt: string;
...
export function HeroCardTooltip({
  title,
  href,
  imgSrc,
  imgAlt,
  className,
  style,
}: HeroCardTooltipProps) {
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <a href={href} className={`hero-card overflow-hidden ${className ?? ''}`} style={style}>
          {imgSrc ? (
